...
def qc_model():
    ...
    model.add(Conv3D(8, (3, 3, 3), activation='relu', input_shape=(None, 192, 256, 256, 1)))
...
def batch(indices, f):
    ...
    while True:
        ...
        for index in indices:
            try:
                yield (images[index, ...][np.newaxis, ..., np.newaxis], labels[index, ..., np.newaxis][np.newaxis, ..., np.newaxis])
            except:
