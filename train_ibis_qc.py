...
def train(epoch, labels):
    ...
    for pass_index, fail_index in zip(pass_indices, fail_indices[0:len(pass_indices)]):
        ...
        target[sample_idx % args.batch_size] = torch.from_numpy(np.asarray([1], dtype='int64'))
        sample_idx += 1

        fail_image, _ = train_dataset[fail_index]
        data[sample_idx % args.batch_size, ...] = torch.from_numpy(fail_image)
        target[sample_idx % args.batch_size] = torch.from_numpy(np.asarray([0], dtype='int64'))
        sample_idx += 1
